...
export class Article {
    ...
    private validateSource(source: string): ArticleSource {
        ...
        throw new InvalidInputError(`source '${source}' does not match any known Article Source`)
    }

    private validateTitle(title: string): string {
        if (title.length > 255) {
            throw new InvalidInputError(`title '${title}' is not less than 255 characters`);
        }
        if (title.length === 0) {
